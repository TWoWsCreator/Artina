...
class PaintingsView(ListView):
    ...
    def get_context_data(self, **kwargs):
        painting = Painting.objects.filter(painting_slug=self.kwargs['painting_slug'])[0]
        return {
...
class LikePaintingView(FormView):
    ...
    def get_queryset():
        return Painting.objects.select_related(
            'painting_artist',
            'painting_gallery').prefetch_related(
            Prefetch(
                'likes',
    ...
    def post(self, request, **kwargs):
        ...
        success_url = reverse_lazy('paintings:painting',
                                   kwargs={'painting_slug': painting_slug})
